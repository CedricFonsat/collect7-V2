...
userRouter.get("/account", async (req, res) => {
  try {
    ...
    let userConnect = await userModel.findOne({ _id: req.session.user });
    res.render("pages/user/account.twig", {
      userConnect: userConnect,
      cards: cards,
    });
  } catch (error) {
...
userRouter.get("/account/update/:id", async (req, res) => {
  try {
    let account = await userModel.findOne({ _id: req.params.id }, req.body);
    res.render("pages/user/update.twig", {
      account: account
    })
  } catch (error) {
    res.send(error);
